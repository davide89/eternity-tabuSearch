...
#awk -f eternity_fine_tuning_winner.awk -v out=10x10_fine_tuning_winner.txt -v scores=10x10_fine_tuning_scores.txt 10x10_fine_tuning_resume.txt
...
($1=="AverageScore:") {
  ...
  if ( (bestTuning==score))  {
    new_best=1;
  } else if ((bestTuning-1.060)<=score) {
    new_range=1;
  }
}